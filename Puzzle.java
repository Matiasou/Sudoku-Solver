 ...
 * Partener Sonya Verma, sonyav@bu.edu
 */
...
public class Puzzle {
    ...
    private boolean solveRB(int n) {
        ...
        for (int i = 0; i < 9; i++){
            ...
            if (!(rowHasValue[row][i+1]) && (!(columnHasValue[column][i+1]))  && (!(subgridHasValue[row/3][column/3][i+1]))) {
                placeVal(i+1, row, column);
                    if (solveRB(n + 1))
                        return true;
                    removeVal(i+1, row, column);
            }
